...
with tf.name_scope("conv_layers"):
    ...
    flat = tf.contrib.layers.flatten(relu1)

with tf.name_scope("FC_layer"):
    W = tf.Variable(tf.random_normal(shape=(784, 10), stddev=0.01), name="FC_W")
    b = tf.Variable(tf.zeros(shape=(1, 10)), name="FC_b")
...
with tf.Session(config=cfg) as sess:
    ...
    batch_idx = 0
    n_batches = int(img_train.shape[0] / batch_size)
    for i in range(n_epochs):
        ...
        print("#{}\tloss: {:5.1f}".format(i, total_loss / n_batches))
